...
import { loadChildReportCard, ChildReportCard } from '../../../../src/lib/supabase/queries/parentReport'
...
export default function ChildReportCardScreen() {
  ...
  useEffect(() => {
    ...
    loadWithCacheFallback(
      `report_card:${learnerId}`,
      () => loadChildReportCard(learnerId)
    )
      .then((result) => {
        setData(result.data)
  ...
  if (loading) return <LoadingState label="Loading report card..." />
  if (error) return <ErrorState message={error} onRetry={() => { setLoading(true); setError(null) }} />
  if (!data) return null
  if (!data.report) return <EmptyState message={data.message ?? 'No result available yet.'} />
