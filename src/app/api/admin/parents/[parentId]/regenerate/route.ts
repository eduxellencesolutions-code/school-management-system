...
export async function POST(
  ...
  if (userRow.organization_id === null || userRow.role !== 'admin') {
    return NextResponse.json({ error: 'Only school admins can regenerate access codes' }, { status: 403 });
  }

  const admin = createAdminClient();

  // Confirm this parent actually belongs to this admin's org (via at least one linked learner)
  ...
  const { error: updateError } = await admin
    .from('parent_accounts')
    .update({ access_code: newCode, access_code_regenerated_at: new Date().toISOString(), access_code_active: true })
    .eq('id', parentId);
