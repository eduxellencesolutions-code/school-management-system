...
export async function GET() {
  ...
  if (userRow.role !== 'admin' && !hasPerm) {
    return NextResponse.json({ error: 'You do not have permission to view promotion rules' }, { status: 403 });
  }
...
export async function POST(request: Request) {
  ...
  if (userRow.role !== 'admin' && !hasPerm) {
    return NextResponse.json({ error: 'You do not have permission to manage promotion rules' }, { status: 403 });
  }
